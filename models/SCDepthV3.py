...
class Model(NetInterface):
    ...
    def add_arguments(cls, parser):
        # model options
        parser.add_argument("--resnet_layers", type=int, default=18, choices=[18, 34, 50, 101, 152], help="number of resnet layers for depth network")
        parser.add_argument("--ImageNet_pretrained", default=True, help="use pretrained weights for depth net")

        # training options
        parser.add_argument("--pose_lr_mul", type=float, default=1, help="lr multiplier for pose network")
    ...
    def __init__(self, opt, logger):
        ...
        self.optimizer_depth = self.optim(self.depth_net.parameters(), lr=opt.lr, **self.optim_params)
        self.optimizer_pose = self.optim(self.pose_net.parameters(), lr=opt.lr * opt.pose_lr_mul, **self.optim_params)
    ...
    def _train_on_batch(self, epoch, batch_idx, batch):
        ...
        if np.mod(epoch, self.opt.vis_every_train) == 0:
            idx = batch_idx if self.opt.vis_at_start else self.opt.epoch_batches - batch_idx
            if idx <= self.opt.vis_batches_train:
                # specify vis how many times in one epoch
                outdir = os.path.join(self.full_logdir, "visualize", "epoch%04d_train" % epoch)
    ...
    def _vali_on_batch(self, epoch, batch_idx, batch):
        ...
        if self.opt.val_mode == "depth":
            errs = LossF.compute_errors(batch["gt_depth"], pred["tgt_depth"], self.opt.dataset)
            errs = {
                "abs_diff": errs[0],
                "abs_rel": errs[1],
                "rmse": errs[4],
            ...
            raise NotImplementedError(f"validation mode {self.opt.val_mod} not supported")
    ...
    def _calc_loss(self, pred):
        ...
        mask_ranking_loss = LossF.mask_ranking_loss(pred["tgt_depth"], self._input.tgt_pseudo_depth, dynamic_mask)
        ...
        total_loss = w1 * photo_loss + w2 * geometry_loss + w3 * normal_l1_loss + w4 * mask_ranking_loss + w5 * normal_ranking_loss
    ...
    def _predict_on_batch(self, is_train=True):
        if is_train:
            ...
            result = {
                ...
                "poses": [self.pose_net(self._input.tgt_img, im) for im in self._input.ref_imgs],
                "poses_inv": [self.pose_net(im, self._input.tgt_img) for im in self._input.ref_imgs],
                "tgt_normal": depth_to_normals(target_depth, self._input.intrinsics),
                "tgt_pseudo_normal": depth_to_normals(self._input.tgt_pseudo_depth, self._input.intrinsics),
            }
        else:
            if self.opt.val_mode == "depth":
